...
final class SRPTests: XCTestCase {
    ...
    func testVerifySRP<H: HashFunction>(configuration: SRPConfiguration<H>) {
        ...
        do {
            ...
            try client.verifyServerProof(serverProof: serverProof, clientProof: clientProof, clientPublicKey: clientKeys.public, sharedSecret: clientSharedSecret)
        } catch {
            XCTFail("\(error)")
        }
    }
    ...
    func testMozillaTestVectors() throws {
        ...
        let clientProof = SHA256.hash(data: SRPKey(A, padding: configuration.sizeN).bytes + SRPKey(B, padding: configuration.sizeN).bytes + sharedSecret.bytes)
